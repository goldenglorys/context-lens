...
const getData = async (id: string): Promise<any> => {
  ...
  const out = new Promise((resolve, reject) => {
    const stream = fs
      .createReadStream("./assets/context_file.jsonl")
      .pipe(ndjson.parse())
      .on("data", async (obj) => {
        if (obj.meta.id === id) {
...
type ContextMeta = {
  id: string;
  run_id: string;
  tiktoken_context_window_length: number;
  claude_context_length: number;
  num_messages: number;
  final_state?: any;
};
const getMetadata = async (): Promise<ContextMeta[]> => {
  ...
  return new Promise((resolve) => {
    fs.createReadStream("./assets/context_file.jsonl")
      .pipe(ndjson.parse())
      .on("data", async (obj) => {
        data.push({
          id: obj.meta.id,
          tiktoken_context_window_length:
            obj.meta.tiktoken_context_window_length,
...
<div class="sticky w-full top-0 bg-white/80 backdrop-blur-md py-4">
  <div class="flex justify-between w-full">
    ...
    <div>
      <a class="bg-pink-200 rounded-md p-2 hover:bg-pink-500 hover:text-white" href="#message-${
        index - 1
      }">Previous (${index})</a>
      <a class="bg-pink-300 rounded-md p-2 hover:bg-pink-500 hover:text-white" href="#message-${
        index + 1
      }">Next (${index + 2})</a>
    </div>
  </div>
